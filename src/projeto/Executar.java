...
public class Executar {
	...
	public static void main(String[] args) {
		...
		Menu m = new Menu();
		// esta variavel vai servir de controlo para termos o menu principal sempre
		// aberto ate alguem dar instrucao para fechar
		boolean menu = true;
		// Criamos a ligacao a classe que faz as ligacoes a base de dados
		Connector c = new Connector();
		// Criamos um scanner para inteiros e um para strings. (Tecnicamente nao
		// precisavas, mas de experiencia costuma dar asneira se nao fizeres)
		Scanner scs = new Scanner(System.in);
		Scanner sci = new Scanner(System.in);
		// este ciclo faz entao com que o menu principal fique aberto enquanto a
		// variavel esteja definida como true
		while (menu == true) {
			int opcao_principal = m.menuPrincipal(); // guarda a tua escolha do menu principal
			switch (opcao_principal) { // este switch funciona como o if e o else if, vai testar a ver qual o valor da
										// tua variavel, neste caso opcao_principal, e conforme o valor dela escolhe o
										// case. Se ela tiver valor 3, executa o case 3, se nao escolher nenhuma opcao
										// valida cai no default.
			case 1: // Menu Clientes
				...
				case 1: // Adicionar Cliente
					...
					if (cl != null) { // garante que foi gerado um cliente com todos os dados.
						...
						c.insereCliente(nome, nif, contacto, email); // utiliza a classe de ligacao a BD para inserir o
																		// cliente na BD
					}
					break;
				case 2: // Apagar Cliente
					...
					int nif_eliminar = c.procuraNIF(nome_eliminar); // Verifica se o Nome do cliente
																					// fornecido consta na base de
																					// dados.
					if (nif_eliminar == 0) // Se a variavel for 0 o cliente nao existe
						...
						c.eliminaCliente(nif_eliminar); // caso contrario a variavel tem o valor do NIF a eliminar e e
														// chamada a funcao, na classe que liga a BD, que o vai
														// eliminar.
					break;
				case 3: // Alterar dados de cliente
					int opcao_alterar = m.menuAlterarCliente(); // chama o menu de alteracoes no cliente
					String nome_alterar; // estas duas variaveis estao iniciadas aqui pois sao utilizadas varias vezes,
											// caso contrario teria de ter um nome diferente para casa vez que as
											// utilizava e iniciava.
					int nif_alterar; // tenho de ter o nome e o NIF porque faco a pesquisa com base no Nome, para
										// facilitar ao utilizador, mas como a chave primaria e o NIF as pesquisas na BD
										// sao feitas atraves deste.
			...
			case 5: // Menu Aniversariantes
				...
				case 1: // Adiciona Aniversariante
					...
					if (an != null) { // garante que foi gerado um aniversariante com todos os dados.
						...
						String data_nascimento = an.getData();
						int id_aniversariante = an.getId_aniversariante();
						// utiliza a classe de ligacao a BD para inserir o cliente na BD
						c.insereAniversariante(id_aniversariante, idade, nome, data_nascimento, alergia); 
					}
					break;
				case 2: // Apagar Aniversariante
					System.out.println("Nome do Aniversariante a apagar: ");
					String nome_eliminar = scs.nextLine();
					// Verifica se o Nome do cliente fornecido consta na base de dados.
					int ID_eliminar = c.procuraID_aniversariante(nome_eliminar); 
					if (ID_eliminar == 0) // Se a variavel for 0 o cliente nao existe
						System.out.println("Aniversariante nao existe");
					else if (ID_eliminar == -1) // se for -1 existiu um erro na procura na BD
						System.out.println("Erro!");
					// caso contrario a variavel tem o valor do NIF a eliminar e e chamada a funcao, na classe que liga a BD, que o  vai eliminar.
					else 
						c.eliminaAniversariante(ID_eliminar); 
					System.out.println("Operacao efectuada");
					break;
				case 3: // Alterar dados de ANIVERSARIANTE
					int opcao_alterar = m.menuAlterarAniversariantes(); // chama o menu de alteracoes no cliente
					String nome_alterar; // estas duas variaveis estao iniciadas aqui pois sao utilizadas varias vezes,
											// caso contrario teria de ter um nome diferente para casa vez que as
											// utilizava e iniciava.
					int id_alterar; // tenho de ter o nome e o NIF porque faco a pesquisa com base no Nome, para
									// facilitar ao utilizador, mas como a chave primaria e o NIF as pesquisas na BD
									// sao feitas atraves deste.
					switch (opcao_alterar) {
					case 1: // Alterar Nome
						System.out.println("Nome do Aniversariante a alterar:");
						nome_alterar = scs.nextLine();
						id_alterar = c.procuraID_aniversariante(nome_alterar); // atraves do nome vai buscar o NIF
						if (id_alterar != 0) { // se o nome tiver um nif associado pede o novo nome.
							System.out.println("Novo Nome:");
							String novo_nome = scs.nextLine();
							c.alteraNome(id_alterar, novo_nome); //isto assim não da...
						} else
					...
					break;
				case 4: // Listar TODOS ANIVERSARIANTES
					c.lerAniversariantes();
					break;
				case 5:
					System.out.println("Nome do Aniversariante a Listar:");
					String nome = scs.nextLine();
					c.lerAniversarianteNome(nome);
				default:
					break;
				}
				break;
